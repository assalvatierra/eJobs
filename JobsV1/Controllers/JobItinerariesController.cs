...
namespace JobsV1.Controllers
{
    public class JobItinerariesController : Controller
    {
        ...
        {
            var jobItineraries = db.JobItineraries.Include(j => j.JobMain).Include(j => j.Destination).OrderBy( d=>d.ItiDate);
            return View(jobItineraries.ToList());
        }

        public ActionResult JobItinerary(int? id)
        {
            var Job = db.JobMains.Where(d => d.Id == id).FirstOrDefault();
            ViewBag.JobOrder = Job;

            var jobItineraries = db.JobItineraries.Include(j => j.JobMain).Include(j => j.Destination).Where(d=>d.JobMainId==id).OrderBy( d=>d.ItiDate);
            return View("Index", jobItineraries.ToList());
        }
        ...
        public ActionResult Create([Bind(Include = "Id,JobMainId,DestinationId,ActualRate,Remarks,ItiDate")] JobItinerary jobItinerary)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Id,JobMainId,DestinationId,ActualRate,Remarks,ItiDate")] JobItinerary jobItinerary)
        {
            if (ModelState.IsValid)
            {
